...
class LPSolver(AbstractSolver):
    ...
    def _generate_variables(
        self, plants: list[PowerPlant], costs: Fuels
        ...
        for plant in plants:
            ...
            variables[f"up_{plant.name}"] = pulp.LpVariable(
                f"up_{plant.name}", lowBound=0, upBound=1, cat=pulp.LpBinary
            )
    ...
    def _generate_problem(
        ...
        for var in [k for k in variables if k.startswith("up_")]:
            ...
            problem += (
                variables[var] * plant.pmin <= variables[f"p_{plant.name}"],
                f"Minimum production for {plant.name}",
            )
            problem += (
                variables[var] * plant.pmax >= variables[f"p_{plant.name}"],
                f"Maximum production for {plant.name}",
            )
        ...
        problem += (
            pulp.lpSum(
                [
                    (variables[f"cost_{plant.name}"] + i * epsilon)
                    * variables[f"p_{plant.name}"]
                    for i, plant in enumerate(plants)
                ]
                + [
                    costs.co2 * 0.3 * variables[f"p_{plant.name}"]
                    for plant in plants
                    if plant.type_ == PlantType.GASFIRED
                ]
            ),
