...
from app.enums import FuelType, PlantType
from app.schemas import Forecast, Fuels, PowerPlant, ProductionPlan
    ...
    def add_cost_to_plants(self) -> list[PowerPlant]:
        ...
        for plant in self.plants:
            if plant.type_ == PlantType.WINDTURBINE:
                ...
                plant.cost = (
                    getattr(self.costs, self.FUEL_LOOKUP[plant.type_])
                    / plant.efficiency
                )
        return self.plants
...
def create_model(forecast: Forecast, solver=None) -> Planner:
    ...
    match solver:
        ...
        case _:
            logger.debug(
                "No solver or backtracking specified.Using naive backtracking solver."
            )
